...
#define delayer_NofCyclesMs(ms, hz)  ((ms)*((hz)/1000)) /* calculates the needed cycles based on bus clock frequency */
#define delayer_NofCyclesUs(us, hz)  ((us)*(((hz)/1000)/1000)) /* calculates the needed cycles based on bus clock frequency */
#define delayer_NofCyclesNs(ns, hz)  (((ns)*(((hz)/1000)/1000))/1000) /* calculates the needed cycles based on bus clock frequency */
...
#define delayer_Waitus(us)  \
       (  ((delayer_NofCyclesUs((us),CLOCK_GetBusClkFreq())==0)||(us)==0) ? \
          (void)0 : \
          ( ((us)/1000)==0 ? (void)0 : delayer_Waitms((uint16_t)((us)/1000))) \
          , (delayer_NofCyclesUs(((us)%1000), CLOCK_GetBusClkFreq())==0) ? (void)0 : \
            delayer_WAIT_C(delayer_NofCyclesUs(((us)%1000), CLOCK_GetBusClkFreq())) \
       )
/*
** ===================================================================
...
#define delayer_Waitns(ns)  \
       (  ((delayer_NofCyclesNs((ns), CLOCK_GetBusClkFreq())==0)||(ns)==0) ? \
          (void)0 : \
          delayer_Waitus((ns)/1000) \
          , (delayer_NofCyclesNs((ns)%1000, CLOCK_GetBusClkFreq())==0) ? \
              (void)0 : \
              delayer_WAIT_C(delayer_NofCyclesNs(((ns)%1000), CLOCK_GetBusClkFreq())) \
       )
/*
** ===================================================================
